...
export const initiateSpotifyLogin = async () => {
  const response = await fetch(`${API_BASE}/auth/login`, { credentials: 'include' });
  return response.json();
};
...
export const getUserProfile = async () => {
  const response = await fetch(`${API_BASE}/spotify/profile`, { credentials: 'include' });
  if (!response.ok) throw new Error('Failed to fetch profile');
  return response.json();
};

export const getTopTracks = async (timeRange: string = 'medium_term', limit: number = 50) => {
  const response = await fetch(`${API_BASE}/spotify/top-tracks?time_range=${timeRange}&limit=${limit}`, { credentials: 'include' });
  if (!response.ok) throw new Error('Failed to fetch top tracks');
  return response.json();
};

export const getTopArtists = async (timeRange: string = 'medium_term', limit: number = 50) => {
  const response = await fetch(`${API_BASE}/spotify/top-artists?time_range=${timeRange}&limit=${limit}`, { credentials: 'include' });
  if (!response.ok) throw new Error('Failed to fetch top artists');
  return response.json();
};

export const getRecentTracks = async (limit: number = 50) => {
  const response = await fetch(`${API_BASE}/spotify/recent?limit=${limit}`, { credentials: 'include' });
  if (!response.ok) throw new Error('Failed to fetch recent tracks');
  return response.json();
};

export const getAudioFeatures = async (trackIds: string) => {
  const response = await fetch(`${API_BASE}/spotify/audio-features?ids=${trackIds}`, { credentials: 'include' });
  if (!response.ok) {
...
export const getMoodTrends = async () => {
  const response = await fetch(`${API_BASE}/mood/trends`, { credentials: 'include' });
...
export const generatePlaylistRecommendation = async (currentMood: string, desiredMood: string, context?: string) => {
  ...
  if (!response.ok) throw new Error('Failed to generate playlist recommendation');
  return response.json();
};